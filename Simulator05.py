...
import numpy as np
import random
import Neurons04 as Neurons
import Project01 as p1
...
P1Neuron = p1.Project01Neuron() # Create an instance of your class here
...
def runSim():
    ...
    '''
    new = prev - (prev -target)(1-exp(-1/tau))
    new = prev-(prev(-1/tau))
    new = prev-prev+prev*exp(-1/tau)

    '''

    if mainWin.simToRun == 1:   # menu selection is "My Simulation"
        while keepgoing:
            for bin in range(1,bin,syn.duration): # NO SYN VARIABLE
                if random.random()<0.01:
                    s = 1
                else:
                    s = 0
                syn.DoTimeStep() # we're going to have to rewatch this aren't we.... :/

            """ for bin in range(1, P1Neuron.duration):
                [Vm, Thresh, sp] = P1Neuron.DoTimeStep(bin)
                mainWin.DrawPoint(bin, Vm, P1Neuron.duration, 0, 50, -80, blue)
                mainWin.DrawPoint(bin, Thresh, P1Neuron.duration, 0, 50, -80, yellow)
                if sp == 1:     # if there is a spike, draw a vertical line
                    mainWin.DrawSpike(bin,Vm,40,P1Neuron.duration,0,50,-80,blue)  """

    # brainstorm: imagine what a threshold would look like
    # trying something and failing before being told
    ...
    ###### Squid axon voltage recording
        ...
        while keepgoing:
            ...
            # Code for mainWin.DrawSweep: 0=full window, 1=top halff, 2=bottom half
            #                             3=top third, 4=middle third, 5= bottom third                              
    ...
    ###### Izhikevich neuron        
        ...
        while keepgoing:             
            a = 0.01 + (random.random()*.02) # use this for random values
            a = 0.02                           # or type your own here
            b = 0.1 + (random.random()*.2)   # use this for random values
            b = 0.2                          # or type your own here
            c = -65
            d = 8
            StimulationIntensity = 6        # determines level of depolarization
            Vm = Ivich.runSim(a,b,c,d,StimulationIntensity) 
    ...
    ###### Leaky integrate-and-fire
        ...
        while keepgoing: 
            for bin in range(1,IAF.duration):
                ...
                mainWin.DrawPoint(bin,Vm,IAF.duration,3,50,-80,blue)

                if s==1: mainWin.DrawSpike(bin,Vm, 1, IAF.duration,3,50,-80,blue)
                mainWin.DrawPoint(bin,Calcium,IAF.duration,4,1,0,orange)
                mainWin.DrawPoint(bin,KCa,IAF.duration,5,1,0,yellow)
##########################
def viewParams():
    p1 = np.zeros(702)
    p2 = np.zeros(702)
    ...
    for v in range(-800,600,2):     
        Vm = (.1 * v)+.1            # step through Vm from -80 to 60
        if counter%50 ==0:
            mainWin.DrawSpike(counter,0,1,700,0,1,0,'gray')
        if counter == 400:
            mainWin.DrawSpike(counter,0,1,700,0,1,0,'white')
        # uncomment one of these to see the params a as function of Vm
        ## H and H squid axon
        ...
        p1[counter] = a/(a+b)                       # n
        p2[counter] = math.pow((a/(a+b)),4)         # n4
        p3[counter] = a1/(a1+b1)                    # m
        p4[counter] = math.pow((a1/(a1+b1)),3)      # m3
        p5[counter] = 1/(a+b)                       # tau n
        p6[counter] = 1/(a1+b1)                     # tau m
        counter += 1
    mainWin.ParmView(p1,magenta)    # n
    mainWin.ParmView(p2, green)     # n4
    mainWin.ParmView(p3, blue)      # m
    mainWin.ParmView(p4, orange)    # m3
    #mainWin.ParmView(p5, white)     # tau n
    #mainWin.ParmView(p6, cyan)      # tau m
MW.mainloop()